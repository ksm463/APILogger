...
#         logger.debug(f"[/send] JSON 데이터 파싱 성공: {body}")
...
#                 f"http://{server_ip}", headers=headers, json=body
#             ) as response:
#                 logger.debug(
#                     f"[/send] 서버로 POST 요청 전송: {server_ip}, 데이터 - {body}"
#                 )
...
async def send_api_request_example(
    ...
    if not example_file_path.exists():
        ...
        raise HTTPException(status_code=400, detail=f"No example.json: {example_file_path}")

    try:
        with open(example_file_path, "r") as data:
            body = json.load(data)
            logger.debug(f"[/sendexample] 파일 읽기 및 JSON 데이터 파싱 성공: {body}")

        async with aiohttp.ClientSession() as session:
            logger.debug(f"[/sendexample] 서버로 POST 요청 전송 시도: URL - http://{server_ip}/testexample, 헤더 - {headers}, 데이터 - {data}")
            async with session.post(
                f"http://{server_ip}/testexample", headers=headers, json=body
            ) as response:
                logger.debug(f"[/sendexample] 서버로 POST 요청 전송: {server_ip}, 데이터 - {body}")
                response.raise_for_status()
                response_data = await response.json()
        ...
        raise HTTPException(status_code=400, detail=f"FileNotFoundError: {e}")
    except Exception as e:
